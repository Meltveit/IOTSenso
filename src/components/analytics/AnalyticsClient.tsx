...
export default function AnalyticsClient({
  ...
  const alertData = last7Days.map((day) => ({
    ...
    alerts: alerts.filter(
      (alert) =>
        format(alert.timestamp, "yyyy-MM-dd") === format(day, "yyyy-MM-dd")
    ).length,
  }));
  ...
  return (
    <div className="flex flex-col gap-6">
        <div className="grid gap-4 md:grid-cols-3">
            <StatCard title="Total Alerts (Last 7 Days)" value={totalAlerts} icon={Bell} />
            <StatCard title="Warning Alerts" value={warningAlerts} icon={ShieldCheck} />
